...
import app_assets
import driver_info
from win_display import change_resolution, get_current_resolution, get_supported_resolutions, test_resolution
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self._footer_logo_photo = None
        self._apply_window_icon_and_footer_assets()
    ...
    def _build(self):
        ...
        if self._footer_logo_photo is not None:
            tk.Label(
                footer_row,
                image=self._footer_logo_photo,
                bg="#21262d",
                bd=0,
    ...
    def _apply_window_icon_and_footer_assets(self):
        # Barra de tarefas / WM: no Windows, .ico via iconbitmap costuma funcionar melhor que PNG/iconphoto.
        ico = app_assets.icon_ico_path()
        ...
        path = app_assets.icon_png_path()
        if not path.is_file():
            return

        # Fallback de título (Linux/macOS ou se .ico não existir no Windows)
        if not (sys.platform == "win32" and ico.is_file()):
            try:
                wm = tk.PhotoImage(master=self, file=str(path))
                while wm.height() > 128:
                    wm = wm.subsample(2, 2)
                self.iconphoto(True, wm)
                self._wm_icon_photo = wm
            except tk.TclError:
                pass

        try:
            foot = tk.PhotoImage(master=self, file=str(path))
            while foot.height() > 36:
                foot = foot.subsample(2, 2)
            self._footer_logo_photo = foot
        except tk.TclError:
            pass
